...
?>
	...
	<?php if ((get_option('unwakeable_rollingarchives') == 0) and !is_single() and !is_home() and is_paged()) include (TEMPLATEPATH . '/navigation.php'); ?> 
	...
	<?php /* Check if there are posts */
		if ( have_posts() ) {
			...
			if(is_archive() or is_search() or is_single() or (function_exists('is_tag') and is_tag())) {
				...
				$k2asidescheck = get_option('unwakeable_asidesposition');
			}
	?>
	...
	<?php /* Insert Paged Navigation */ if (!is_single() and get_option('unwakeable_rollingarchives') != 1) { include (TEMPLATEPATH.'/navigation.php'); } ?>
