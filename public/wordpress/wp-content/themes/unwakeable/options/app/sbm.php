...
class k2sbm {
	...
	function add_menus() {
		// Add the submenus
		add_submenu_page('themes.php', __('Unwakeable Sidebar Modules', 'k2_domain'), __('Unwakeable Sidebar Modules', 'k2_domain'), 5, 'k2sbm-modules', array('k2sbm', 'module_admin'));
	}
	...
	function load_modules() {
		...
		if(empty($k2sbm_active_modules) and empty($k2sbm_disabled_modules)) {
			$k2sbm_active_modules = get_option('unwakeable_sbm_modules_active');
			$k2sbm_disabled_modules = get_option('unwakeable_sbm_modules_disabled');
		}
	}
	...
	function save_modules() {
		...
		update_option('unwakeable_sbm_modules_active', $k2sbm_active_modules);
		update_option('unwakeable_sbm_modules_disabled', $k2sbm_disabled_modules);
	}
	...
	function add_module($name, $type, $sidebar) {
		...
		if($base_module) {
			...
			// Quick & cheap
			$next_id = get_option('unwakeable_sbm_modules_next_id');
			$module_id = 'module-' . $next_id;
			update_option('unwakeable_sbm_modules_next_id', ++$next_id);
